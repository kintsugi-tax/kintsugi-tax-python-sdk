"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from kintsugi_tax_platform_sdk.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
from kintsugi_tax_platform_sdk.utils import (
    FieldMetadata,
    HeaderMetadata,
    QueryParamMetadata,
)
import pydantic
from pydantic import model_serializer
from typing import Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class GetProductsV1ProductsGetRequestTypedDict(TypedDict):
    x_organization_id: Nullable[str]
    r"""The unique identifier for the organization making the request"""
    query: NotRequired[Nullable[str]]
    r"""Search term to filter products by name or other details."""
    status_in: NotRequired[Nullable[str]]
    r"""Filter products by status (comma-separated)"""
    product_category_in: NotRequired[Nullable[str]]
    r"""Filter products by category (comma-separated)"""
    product_subcategory_in: NotRequired[Nullable[str]]
    r"""Filter products by subcategory (comma-separated)"""
    source_in: NotRequired[Nullable[str]]
    r"""Filter products by source (comma-separated)"""
    connection_id_in: NotRequired[Nullable[str]]
    r"""Filter products by connection ID (comma-separated). Use __direct_api__ for products without a connection."""
    order_by: NotRequired[Nullable[str]]
    r"""Order results by specified fields (comma-separated)"""
    page: NotRequired[int]
    r"""Page number"""
    size: NotRequired[int]
    r"""Page size"""


class GetProductsV1ProductsGetRequest(BaseModel):
    x_organization_id: Annotated[
        Nullable[str],
        pydantic.Field(alias="x-organization-id"),
        FieldMetadata(header=HeaderMetadata(style="simple", explode=False)),
    ]
    r"""The unique identifier for the organization making the request"""

    query: Annotated[
        OptionalNullable[str],
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = UNSET
    r"""Search term to filter products by name or other details."""

    status_in: Annotated[
        OptionalNullable[str],
        pydantic.Field(alias="status__in"),
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = UNSET
    r"""Filter products by status (comma-separated)"""

    product_category_in: Annotated[
        OptionalNullable[str],
        pydantic.Field(alias="product_category__in"),
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = UNSET
    r"""Filter products by category (comma-separated)"""

    product_subcategory_in: Annotated[
        OptionalNullable[str],
        pydantic.Field(alias="product_subcategory__in"),
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = UNSET
    r"""Filter products by subcategory (comma-separated)"""

    source_in: Annotated[
        OptionalNullable[str],
        pydantic.Field(alias="source__in"),
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = UNSET
    r"""Filter products by source (comma-separated)"""

    connection_id_in: Annotated[
        OptionalNullable[str],
        pydantic.Field(alias="connection_id__in"),
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = UNSET
    r"""Filter products by connection ID (comma-separated). Use __direct_api__ for products without a connection."""

    order_by: Annotated[
        OptionalNullable[str],
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = UNSET
    r"""Order results by specified fields (comma-separated)"""

    page: Annotated[
        Optional[int],
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = 1
    r"""Page number"""

    size: Annotated[
        Optional[int],
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = 50
    r"""Page size"""

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(
            [
                "query",
                "status__in",
                "product_category__in",
                "product_subcategory__in",
                "source__in",
                "connection_id__in",
                "order_by",
                "page",
                "size",
            ]
        )
        nullable_fields = set(
            [
                "query",
                "status__in",
                "product_category__in",
                "product_subcategory__in",
                "source__in",
                "connection_id__in",
                "order_by",
                "x-organization-id",
            ]
        )
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))
            is_nullable_and_explicitly_set = (
                k in nullable_fields
                and (self.__pydantic_fields_set__.intersection({n}))  # pylint: disable=no-member
            )

            if val != UNSET_SENTINEL:
                if (
                    val is not None
                    or k not in optional_fields
                    or is_nullable_and_explicitly_set
                ):
                    m[k] = val

        return m
